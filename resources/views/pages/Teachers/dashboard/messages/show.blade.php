...
@section('css')
    @toastr_css
    @section('title')
        محادثة مع {{ $parent->getTranslation('Name_Father', 'ar') }}
    @stop
@endsection
@section('page-header')
    @section('PageTitle')
        ...
        <div class="col-md-10 offset-md-1 mb-30">
            <div class="card card-statistics h-100">
                <div class="card-header bg-success text-white">
                    <a href="{{ route('teacher.messages.index') }}" class="btn btn-sm btn-light ml-2">
                        <i class="fas fa-arrow-right"></i> رجوع
                    </a>
                    <span style="font-family: 'Cairo', sans-serif">
                        <i class="fas fa-user-tie"></i>
                        محادثة مع ولي الأمر: {{ $parent->getTranslation('Name_Father', 'ar') }}
                    </span>
                </div>
                <div class="card-body" style="font-family: 'Cairo', sans-serif; min-height: 400px;">
                    <div id="messages-container" style="max-height: 500px; overflow-y: auto; padding: 10px; background-color: #f5f5f5; border-radius: 8px;">
                        @forelse($messages as $msg)
                            @php
                                $isMine = ($msg->sender_type === 'teacher' && $msg->sender_id == auth()->user()->id);
                            @endphp
                            <div class="d-flex mb-3 {{ $isMine ? 'justify-content-end' : 'justify-content-start' }}">
                                <div class="card {{ $isMine ? 'bg-success text-white' : 'bg-white' }}"
                    ...
                    <hr>
                    <form action="{{ route('teacher.messages.store', $parent->id) }}" method="post">
                        @csrf
                        <div class="form-group">
                            <label><i class="fas fa-keyboard text-success"></i> ردك</label>
